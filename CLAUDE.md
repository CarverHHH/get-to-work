...
| `workflow-plugin/state.json` | 当前工作流状态（schema v2，Claude 用 Read/Write 维护） |
...
| `workflow-plugin/memory/` | 知识库（specs / plans / adr / lessons / patterns） |
...
- **路径基准**：所有引用从 repo root（`get-to-work/`）起算
- **state.json**：schema v2，`history[]` 最多 10 条
